      ...
      <body className="bg-slate-900 text-white">
        <div className="flex h-screen bg-slate-900">
          ...
          <main className="flex-1 flex flex-col overflow-hidden">
            {/* Mobile Header */}
            <header className="lg:hidden bg-slate-800 border-b border-slate-700 px-4 py-4 flex items-center justify-between">
...
function NavLink({ href, icon, label }) {
  return (
    <Link
      href={href}
      className="nav-link text-gray-300 hover:text-white active:bg-green-600 active:text-white"
    >
...
function MobileNavLink({ href, icon, label }) {
  return (
    <Link
      href={href}
      className="flex flex-col items-center justify-center gap-1 py-2 px-3 text-gray-400 hover:text-white hover:bg-slate-700 rounded-lg transition-colors text-center"
      title={label}
    >
